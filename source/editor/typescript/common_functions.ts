...
export function getRTInstances(sys: FlameSystem, component_name: string): RuntimeComponent[] {
    return Array.from(sys.document.getElementsByClassName(component_name)).map(comp => comp.wick_component);
}


export function getRTInstanceFromEvent(event) {
    return retrieveRTInstanceFromElement(getElementFromEvent(event));
}
...
export function getElementFromPoint(x: number, y: number) {
...
export function getElementFromEvent(event) {
    return getElementFromPoint(event.x, event.y);
}

export function getIndexOfElementInRTInstance(comp: RuntimeComponent, ele: HTMLElement): number {
    //@ts-ignore
